    ...
    <div class="container-login">
        ...
        <form action="login.php" method="POST">
            ...
            <div class="form-footer">
                ...
                <p>Forgotten Password? <a href="forgotemail.php">Reset Password</a></p>
            </div>
        </form>
    </div>
</div>
    ...
    if ($userResult && mysqli_num_rows($userResult) > 0) {
        ...
        if (password_verify($password, $user['passwords']) || $password === $user['passwords']) {
            ...
            if ($stmt) {
                ...
                error_log("Failed to prepare query for resetting failed attempts: " . $conn->error);
            }
            ...
            echo "<script>alert('Login successful! Welcome back.'); window.location.href = 'menu.php';</script>";
            exit;
        } else {
            ...
            if ($stmt) {
                ...
                if ($user['failed_attempts'] + 1 >= 3) {
                    ...
                    if ($stmt) {
                        ...
                        error_log("Failed to prepare query for blocking user account: " . $conn->error);
                    }
                } else {
                    echo "<script>alert('Invalid username or password.'); window.location.href = 'login.php';</script>";
                }
            } else {
                error_log("Failed to prepare query for incrementing failed attempts: " . $conn->error);
            }
        }
    } else {
        echo "<script>alert('Invalid username or password.'); window.location.href = 'login.php';</script>";
