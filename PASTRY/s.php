    ...
    if ($check_user_stmt->num_rows > 0) {
        ...
        if ($insert_user_stmt->execute()) {
            ...
            if (sendVerificationEmail($email, $otp_code, $fullname)) {
                echo "<script>alert('Registration successful! Please check your email for verification code.'); window.location.href='verify_email.php';</script>";
            } else {
                // If email fails, still create account but inform user
                echo "<script>alert('Account created but there was an issue sending the verification email. Please contact support.'); window.location.href='login.php';</script>";
            }
        } else {
...
function sendVerificationEmail($email, $otp_code, $fullname) {
    ...
    try {
        ...
        $mail->Password = 'blqdboqbhlrhqxxc';
