...
<head>
  ...
  <style>
    ...
    .edit-btn, .delete-btn, .view-btn, .unblock-btn {
      padding: 5px 10px;
...
<body>
  ...
  <div class="main-content">
    ...
    <?php if(isset($_SESSION['success_message'])): ?>
      <div class="alert success" style="background-color: #dff0d8; color: #3c763d; padding: 15px; margin-bottom: 20px; border-radius: 4px; border: 1px solid #d6e9c6;">
    ...
    <?php if(isset($_SESSION['error_message'])): ?>
      <div class="alert error" style="background-color: #f2dede; color: #a94442; padding: 15px; margin-bottom: 20px; border-radius: 4px; border: 1px solid #ebccd1;">
        ...
        <tbody>
          <?php while($user = $users->fetch_assoc()): ?>
            <tr>
              ...
              <td><?= $user['email'] ?></td>
              <td class="<?= strtolower($user['status']) == 'inactive' ? 'status-inactive' : 'status-active' ?>">
                <?= $user['status'] ?>
              </td>
              <td class="action-btns">
                ...
                <!-- Always show the unblock button for inactive users -->
                <?php if(strtolower($user['status']) == 'inactive'): ?>
                <button class="unblock-btn" onclick="openUnblockModal(<?= $user['AccountID'] ?>)">Unblock</button>
                <?php endif; ?>
  ...
  <div id="unblockModal" class="modal">
    <div class="modal-content">
      <span class="close-btn" onclick="closeUnblockModal()">&times;</span>
      <h3 style="color: var(--primary-color); margin-bottom: 20px;">Unblock User</h3>
      <p>Are you sure you want to unblock this user? This will change their status to active.</p>
      <form id="unblockForm" method="POST" action="unblock_user.php">
        ...
        <div style="text-align: right; margin-top: 20px;">
          <button type="submit" class="submit-btn" style="background-color: #4caf50;">Unblock</button>
          <button type="button" class="submit-btn" style="background-color: #ddd; color: #333; margin-left: 10px;" onclick="closeUnblockModal()">Cancel</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <script>
    ...
    function openUnblockModal(id) {
      document.getElementById('unblockAccountId').value = id;
